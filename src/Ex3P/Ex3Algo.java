...
public class Ex3Algo implements PacManAlgo {
	...
	public int move(PacmanGame game) {
		...
		if (bestDir5 == -1) { // מוצא אחרון אם הכל מסוכן
	...
	public double evaluatedir(Pixel2D pos, Map map, int[][] board, double[][] isdanger) {
		...
		if (pink != null) {
			...
			ans += 200000.0 / (d + 1); // הגדלת המשקל של האוכל
		} else {
			ans += ghostDist11 * 5000; // אם אין אוכל, פשוט תתרחק מהרוח
		}
	...
	public Pixel2D closestPINK(int[][] board, Map2D dist, int color) {
		Pixel2D Rbest = null; double min = Double.MAX_VALUE;
		for (int i = 0; i < board.length; i++)
			for (int j = 0; j < board[0].length; j++)
				if (board[i][j] == color && dist.getPixel(i, j) != -1 && dist.getPixel(i, j) < min) {
					min = dist.getPixel(i, j); Rbest = new Index2D(i, j);
				}
		return Rbest;
	}
	...
	public Pixel2D cheksneighbor(Pixel2D p, int direct, Map map) {
		...
		if (direct == Game.UP) y++; else if (direct == Game.DOWN) y--;
		else if (direct == Game.LEFT) x--; else if (direct == Game.RIGHT) x++;
	...
	public String keytohash(Pixel2D p) { return p.getX() + "," + p.getY(); }
}
